...
namespace Client.WebUI.Areas.Conveyor.Controllers
{
    ...
    {
        //
        // GET: /Conveyor/LocationTask/

        [WmsOperation("输送线.货位任务")]
        ...
        [HttpPost]
        public ActionResult Index_List(LocationTaskSpec spec, FormCollection form)
        {
            ...
            try
            {
                using (NhRepositoryContext context = NhRepositoryContextFactory.Instance.CreateRepositoryContext())
                {
                    using (NhUnitOfWork unitOfWork = context.BeginUnitOfWork())
                    {
                        ...
                        var repository = factory.Create<ConveyorRepository<LocationTask>>();
                        ...
                        ViewData.Model = new PagedList.StaticPagedList<LocationTask>(list, pageInfo.PageIndex, pageInfo.PageSize, pageInfo.TotalItemCount);
