(function () {
    ...
    Kinetic.JSConveyorLocation.prototype = {
                ...
                sceneFunc: function (context) {
                    ...
                    try {
                        if (that.getStatus() != null && that.getStatus() != undefined) {
                            var fillColor = that.getStatusColors()[that.getStatus().toString()];
                            that.getMaskShape().fill(fillColor);
                            this.setStroke('#ccc');
                        }
                    } catch (e) {
                    ...
                    content: {
                        text: function () {
                            return '<div style="line-height:1.8">' +
                                        '<label class="title" style = "width: 60px;display:inline-block;";>货位号：</label><label name="HWH" class="content">-</label><br />' +
                                        '<label class="title" style = "width: 60px;display:inline-block;";>状态：</label><label name="ZT" class="content">-</label><br />' +
                                        '<label class="title" style = "width: 60px;display:inline-block;";>任务号：</label><label name="RWH" class="content">-</label><br />' +
                                        '<label class="title" style = "width: 60px;display:inline-block;";>占位：</label><label name="ZW" class="content">-</label><br />' +
                                        '<label class="title" style = "width: 60px;display:inline-block;";>故障：</label><label name="GZ" class="content">-</label><br />' +
                                        '<label class="title" style = "width: 60px;display:inline-block;";>光电：</label><label name="GD" class="content">-</label><br />' +
                                    '</div>';
                        },
                        title: {
                            text: that.getUserCode(),
                            button: true
                        }
                    },
                    position: {
        ...
        getOccupyDescription: function (v) {
            var r = "";
            if (v.FroProPotocell) {
                r += " 保护光电";
            }
        ...
        getAlarmDescription: function (v) {
            var r = "";
            if (v.Manual) {
                r += " 手动";
            }
            ...
            if (v.X_MotorContactor) {
                r += " X轴电机正反转接触器故障";
            }
            if (v.X_MotorBraker) {
                r += " X轴电机抱闸接触器故障";
            }
            if (v.Y_MotorContactor) {
                r += " Y轴电机正反转接触器故障";
            }
            if (v.Y_MotorBraker) {
                r += " Y轴电机抱闸接触器故障";
            }
            if (v.Lift_MotorContactor) {
                r += " 顶升电机正反转接触器故障";
            }

            if (v.Lift_MotorBraker) {
                r += " 顶升电机抱闸接触器故障";
            }
                ...
                if (qtip && qtip.hasOwnProperty('_id')) {
                    ...
                    $('#' + tipId + ' label[name]').each(function (i, item) {
                        switch ($(item).attr('name')) {
                            case 'HWH':
                                $(item).text(that.getDeviceCode());
                                break;
                            case 'ZT':
                                var stat = that.getStatus();
                                if (stat) {
                                    $(item).text(that.getStateName(stat));
                                } else {
                                    $(item).text('-');
                                }
                                break;
                            case 'RWH':
                                var tsk = that.getTask();
                                if (tsk) {
                                    //$(item).text('TaskNo:' + tsk.TaskNo + ' TUID:' + tsk.TUID);
                                    $(item).text(tsk.TaskNo);
                                } else {
                                    $(item).text('-');
                                }
                                break;
                            case 'ZW':
                                var holdSignal = that.getHoldSignal();
                                if (holdSignal) {
                                    if (holdSignal.HandShake == 1) {
                                        //$(item).text('有占位 ' + 'TUID:' + holdSignal.TU_ID + 'TU_TYPE:' + holdSignal.TU_Type + 'IO_DATA:' + holdSignal.IO_Data);
                                        $(item).text('有占位');
                                    } else {
                                        $(item).text('无占位');
                                    }
                                ...
                                break;
                            case 'GD':
                                var occupy = that.getOccupy();
                                if (occupy) {
                                    $(item).text(that.getOccupyDescription(occupy));
                                } else {
                                    $(item).text('-');
                                }
                                break;
                            case 'GZ':
                                var alarm = that.getAlarm();
                                if (alarm) {
                                    $(item).text(that.getAlarmDescription(alarm));
                                } else {
                                    $(item).text('-');
                                }
                                break;
        ...
        _addListeners: function (text) {
            ...
            this.on('taskChange.kinetic', function () {
                this.updateTip();
            });
            this.on('occupyChange.kinetic', function () {
                this.updateTip();
            });
            this.on('alarmChange.kinetic', function () {
                this.updateTip();
            });
            this.on('holdSignalChange.kinetic', function () {
                this.updateTip();
            });
        },
        _sync: function () {
    ...
    Kinetic.Factory.addGetterSetter(Kinetic.JSConveyorLocation, 'holdSignal',
    {
        "HandShake":0,
        "AssignmentID":0,
        "TU_ID":0,
