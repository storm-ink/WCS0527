...
{
    public partial class uc堆垛机_状态数据_年 : UserControl, IChart
    {
        ...
        void load()
        {
            ...
            {   
                //按照所选的时间段的 前5年 
                double totalYears = 5;
                chart1.ChartAreas[0].AxisX.Maximum = totalYears;          
            }
        ...
        void loadLineChart(IEnumerable<StateExecutionTime> datas, double fromYear, double toYear)
        {
            ...
            #region 设置数据
            chart1.ChartAreas[0].AxisX.Minimum = fromYear;
            chart1.ChartAreas[0].AxisX.Maximum = toYear;
            #endregion
            ...
            if (rbCountType_Num.Checked)
            {
                ...
                {
                    chart1.Titles[0].Text = string.Format("{0} 5年堆垛机状态变化次数", fromYear);
                }
                else
                {

                    chart1.Titles[0].Text = string.Format("{0:yyyy年}{1:00}月 至 {2:yyyy年}{3:00}月堆垛机状态变化次数", this.StartTime, fromYear, this.StartTime, toYear);
                }
            }
            else
            {
                if (toYear - fromYear == 5)
                {
                    chart1.Titles[0].Text = string.Format("{0}年 堆垛机状态持续时间", fromYear);
                }
                else
                {
                    chart1.Titles[0].Text = string.Format("{0:yyyy年}{1:00}月 至 {2:yyyy年}{3:00}月堆垛机状态持续时间", this.StartTime, fromYear, this.StartTime, toYear );
                }
            }
            #endregion
            ...
            for (double i = fromYear; i <= (toYear - fromYear == 1 ? fromYear : toYear); i += 1)
            {
            ...
            #region 添加 x 轴刻度
            double byYearCountInterval = 0.5;
            if (toYear - fromYear < 2)
            {
                for (double i = fromYear; i < toYear; i=i+ byYearCountInterval)
                {
                    var months = (i - fromYear) * 5;

                    var curstomLabel = chart1.ChartAreas[0].AxisX.CustomLabels.Add(i, i + byYearCountInterval, string.Format("{0:00}:00", months));
                    curstomLabel.GridTicks = System.Windows.Forms.DataVisualization.Charting.GridTickTypes.Gridline;
                }

                chart1.ChartAreas[0].AxisX.Interval = byYearCountInterval;
            }
            else
            {
            ...
            {
                times = datas.Where(x => x.FromState.StartTime.Year == fromYear);
            }
            else
            {
                times = datas.Where(x => x.FromState.StartTime.Year >= fromYear && x.FromState.StartTime.Month <= toYear);
            }
            ...
            foreach (var itemGrouping in times.GroupBy(x => x.DeviceName).OrderBy(x => x.Key))
            {
                ...
                //按年统计
                for (double year = fromYear; year <= (toYear - fromYear == 5 ? fromYear : toYear); year++)
                {
                    if (toYear - fromYear == 1)
                    {
                        double stepValue = byYearCountInterval*12;
                        //每一年显示一个点
                        for (double years = 0; years < 5; years += stepValue)
                        {
                            if (rbCountType_Num.Checked)
                            {
                                
                                var yValue = itemGrouping
                                    .Where(x => x.FromState.StartTime.Year == year
                                        && x.FromState.StartTime.Subtract(new DateTime(x.FromState.StartTime.Date.Year, Convert.ToInt32(year), 1)).TotalHours >= years
                                        && x.FromState.StartTime.Subtract(new DateTime(x.FromState.StartTime.Date.Year, Convert.ToInt32(year), 1)).TotalHours < years + stepValue
                                        )
                                    .Count();
                                var xValue = fromYear + (years + stepValue / 2) / 12d;
                            ...
                            else
                            {
                                ...
                                var xx=new DateTime(StartTime.Year,Convert.ToInt32(year),1);
                               var yy=xx.AddMonths(1);
                                var xValue = fromYear + (years + stepValue/2) / yy.Subtract(xx).TotalDays;
                    ...
                    else
                    {
                        ...
                        {
                            var yValue = itemGrouping
                                    .Where(x => x.FromState.StartTime.Month == year)
                                    .Sum(x => x.TotalSeconds);
                            var xValue = year + 1 / 2d;
        ...
        private void btnPrevDay_Click(object sender, EventArgs e)
        {
            ...
            dateTimePicker1.Value = this.StartTime.Date.AddYears(-5);
