...
namespace Aga.Controls.Tree.NodeControls
{
	public abstract class BaseTextControl : EditableControl
	{
		...
		{
            return GetLabelSize(GetLabel(node), node.Font ?? this.Font);
		}
		...
		public override void Draw(TreeNodeAdv node, DrawContext context)
		{
            ...
            {
                context.Graphics.FillRectangle(new SolidBrush(node.BackColor), context.Bounds);
            }

			Brush text = SystemBrushes.ControlText;
            if(!node.ForeColor.IsEmpty)
            {
                text = new SolidBrush(node.ForeColor);
            }
			string label = GetLabel(node);
			Size s = GetLabelSize(label,node.Font??this.Font);
            ...
            context.Graphics.DrawString(label, node.Font??context.Font, text, clipRect, _format);
		}

		protected virtual string GetLabel(TreeNodeAdv node)
		{
            if (node.GetLable != null)
            {
                String label = node.GetLable(this,node);
