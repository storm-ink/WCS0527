...
namespace Wcs.App.Plugins.TaskManager
{
    public partial class frmTaskViewer : Form
    {
        ...
        void load(Int32 taskId)
        {
            ...
            btnChangeEndLocation.Visible = Wcs.Framework.Cfg.WcsConfiguration.Instance.SettingCollection.GetSetting<bool>("AbleTaskChouseEndLocation", false);

            setButtonsStatus(task.Status);

            _movementsBindingSource.Clear();
            foreach (var item in task.Movements.OrderBy(x=>x.Ordering))
            {
        ...
        private void tsmiArchive_Click(object sender, EventArgs e)
        {
            try
            {
                if (MessageBox.Show("是否确认要归档任务？", Application.ProductName, MessageBoxButtons.YesNo, MessageBoxIcon.Question) != System.Windows.Forms.DialogResult.Yes)
                {
                    return;
                }
            ...
            {
                _logger.Error1(ex, this);
                MessageBox.Show(string.Format("任务归档失败：{0}", ex.Message), Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void tsmiSuspend_Click(object sender, EventArgs e)
        {
            try
            {
                if (MessageBox.Show("是否确认要暂停任务？", Application.ProductName, MessageBoxButtons.YesNo, MessageBoxIcon.Question) != System.Windows.Forms.DialogResult.Yes)
                {
            ...
            {
                _logger.Error1(ex, this);
                MessageBox.Show(string.Format("任务暂停失败：{0}", ex.Message), Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void tsmiCancel_Click(object sender, EventArgs e)
        {
            ...
            {
                _logger.Error1(ex, this);
                MessageBox.Show(string.Format("任务取消失败：{0}", ex.Message), Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void tsmiComplete_Click(object sender, EventArgs e)
        {
            ...
            {
                _logger.Error1(ex, this);
                MessageBox.Show(string.Format("任务强制完成失败：{0}", ex.Message), Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void btnResume_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (task == null)
                {
                    MessageBox.Show(string.Format("未找到 id 为 {0} 的任务", _taskId), Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
            ...
            {
                _logger.Error1(ex, this);
                MessageBox.Show(string.Format("任务继续执行失败：{0}", ex.Message), Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void btnChangeEndLocation_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!path)
                {
                    MessageBox.Show(String.Format("{0}到新的任务终点{1}不连通。", task.CurrentLocation, newEndLocation), Application.CompanyName, MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
            ...
            {
                _logger.Error1(ex, this);
                MessageBox.Show(string.Format("调整任务终点失败：{0}", ex.Message), Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
    }
}
